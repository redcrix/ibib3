(function() {
    ...
    projectCostByte.controller('WastageLiquorDetailCtrl', wastageLiquorDetailCtrl);

    wastageLiquorDetailCtrl.$inject = ['$scope', 'CommonService', '$state', 'CommonConstants','$http'];

    function wastageLiquorDetailCtrl($scope, CommonService, $state, CommonConstants, $http) {
        $scope.initializeView = function() {
            //$scope.navBarTitle.showToggleButton = true;
            $scope.number = $state.params.id;
            $scope.navBarTitle.showToggleButton = true;

            var data = {
                "chartData" : [[723.70, 545.10, 618.80, 630.40, 652.60, 535.00], [723.70, 545.10, 618.80, 630.40, 652.60, 535.00]],
                "chartDates" : ["Oct 2015", "Nov 2015", "Dec 2015", "Jan 2016", "Feb 2016", "March 2016"],
            ...
            $scope.chartItems = data.chartLiquorItems;
        ...
        $scope.$on('WASTAGELIQUORDETAIL_DATECHANGE_EVENT',function(event){
